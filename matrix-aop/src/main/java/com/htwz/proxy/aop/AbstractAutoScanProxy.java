...
import com.htwz.proxy.constant.ProxyConstant;
import com.htwz.proxy.mode.ProxyMode;
import com.htwz.proxy.mode.ScanMode;
import com.htwz.proxy.util.ProxyUtil;
...
public abstract class AbstractAutoScanProxy extends AbstractAutoProxyCreator {
    ...
    AbstractAutoScanProxy(ProxyMode proxyMode, ScanMode scanMode) {
        this((String[]) null, proxyMode, scanMode);
    }

    AbstractAutoScanProxy(String scanPackages, ProxyMode proxyMode, ScanMode scanMode) {
    ...
    AbstractAutoScanProxy(ProxyMode proxyMode, ScanMode scanMode, boolean exposeProxy) {
        this((String[]) null, proxyMode, scanMode, exposeProxy);
    }

    AbstractAutoScanProxy(String scanPackages, ProxyMode proxyMode, ScanMode scanMode, boolean exposeProxy) {
    ...
    AbstractAutoScanProxy(String[] scanPackages, ProxyMode proxyMode, ScanMode scanMode, boolean exposeProxy) {
        ...
        log.info("-------------Distributed lock Aop Information ------------");
            ...
            if (proxied != null) {
                ...
                switch (proxyMode) {
                    ...
                    case BY_CLASS_ANNOTATION_ONLY:
                        proxyInterceptors = scanAndProxyForClass(targetClass, interceptors);
                        break;
                    // 只通过扫描到接口或者类方法上的注解后，来确定是否要代理
                    case BY_METHOD_ANNOTATION_ONLY:
                        proxyInterceptors = scanAndProxyForMethod(targetClass, interceptors);
                        break;
                    // 上述两者都可以
                    case BY_CLASS_OR_METHOD_ANNOTATION:
                        Object[] classProxyInterceptors = scanAndProxyForClass(targetClass, interceptors);
                        // 没有接口或者类名上扫描到目标注解，那么扫描接口或者类的方法上的目标注解
                        Object[] methodProxyInterceptors = scanAndProxyForMethod(targetClass, interceptors);
    ...
    private Object[] scanAndProxyForClass(Class<?> targetClass, Object[] interceptors) {
        // 判断目标注解是否标注在接口名或者类名上
    ...
    private Object[] scanAndProxyForMethod(Class<?> targetClass, Object[] interceptors) {
        // 判断目标注解是否标注在方法上
        boolean proxied = false;
     ...
     * 是否是“只扫描指定目录”的方式
     */
    private boolean scanPackagesEnabled() {
